...
const Blog = (props) => {
  ...
  return (
    <Layout>
      <SEO title={blogPost.title} description={blogPost.description} image={blogPost.image.publicURL} article={true} />

      <article itemscope itemtype="http://schema.org/Article">
        <header css={css`padding: 0 ${rhythm(2/3)};`}>
          ...
          <Container fluid css={css`padding: 0;`}>
            <Row noGutters={true}>
              <Col>
                <ArticleInfo>Published on <span itemprop="datePublished">{blogPost.date}</span></ArticleInfo>
              </Col>
            </Row>
            <Row noGutters={true}>
              <Col xs={9}>
                <ArticleInfo>by <span itemprop="author">{blogPost.author}</span></ArticleInfo>
              </Col>
              <Col xs={3}>
        ...
        <Img css={css`margin-bottom: ${rhythm(1/4)};`}
          fluid={blogPost.image.childImageSharp.fluid}
          alt="Associated with the post."
        />
        <ImageRef>{blogPost.imageCredits}</ImageRef>
        <div css={css`padding: ${rhythm(2/3)};`} dangerouslySetInnerHTML={{ __html: props.data.markdownRemark.html }}/>
      </article>
